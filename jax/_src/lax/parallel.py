...
def _all_to_all_effectful_abstract_eval(
  ...
  return (
      input_aval.update(shape=tuple(shape), weak_type=False),
...
def _all_gather_effectful_abstract_eval(
  ...
  return (
      x_aval.update(shape=new_shape, named_shape=new_named_shape),
      {*map(core.NamedAxisEffect, axis_name)},
  )
...
def _axis_index_effectful_abstract_eval(*, axis_name):
  ...
  return (
      ShapedArray((), np.int32, named_shape={axis_name: frame.size}),
      {core.NamedAxisEffect(axis_name)},
  )
...
def _pdot_effectful_abstract_eval(
  ...
  named_shape = {name: size
                 ...
                 if name not in axis_name}
  return (
      pos_aval.update(named_shape=named_shape),
      {*map(core.NamedAxisEffect, axis_name)},
  )
